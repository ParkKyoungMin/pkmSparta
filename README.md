...
- **사용자 입력 처리**: 사용자로부터 숫자와 연산자를 입력받고, 이를 바탕으로 계산합니다.
- **예외 처리**: 잘못된 입력에 대해 적절한 오류 메시지를 출력합니다.
- **캡슐화**: 메모리에 저장된 데이터를 캡슐화시켜 직접적인 접근을 막았습니다.
...
```
App.java의 코드에서 setMemory 오른쪽의 **0** 이라는 숫자는 메모리에 저장된 값에서 바꾸고 싶은 위치를 의미하며
ex [1 , 2, 3] 의 리스트에서 1은 0번 index, 2는 1번 index 값을 가집니다.
"1 + 9 = 10" 은 바꾸고 싶은 값을 의미하며 원하는 산식을 기입하면 수정이 반영됩니다.


1. **프로그램 실행**: `App.java` 클래스를 실행하여 계산기를 시작합니다.
2. **입력**: 
   - 첫 번째 숫자
   - 두 번째 숫자
   - 사칙연산 기호
...
- 현재 저장된 메모리: [10 + 5 = 15]
- 현재 저장된 메모리: [1 + 9 = 10]
- 저장된 첫번째 데이터가 삭제되었습니다.
- 현재 저장된 메모리: []
- 계속하시려면 아무키나 입력해주세요.(exit 입력시 종료)
```
